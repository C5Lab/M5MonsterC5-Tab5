...
assert re.search(r"#define\s+JANOS_USB_FT_FAST_BAUD\s+921600\b", SOURCE), (
    "USB fast sync must use the experimental 921600 baud rate"
)
...
static unsigned set_baud_calls, restore_calls, upload_calls;
...
{
    set_baud_calls = restore_calls = upload_calls = 0;
...
int main(void)
{
    ...
    CHECK(set_baud_rate == 921600 && restore_baud_rate == 921600);
